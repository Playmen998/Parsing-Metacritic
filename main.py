...
if __name__ == "__main__":
    ...
    # try:
    #     start_time = time.time()
    #     name_game = game[0] #name_game = game, везде!
    #     platform = game[1]
    #     index = edit_name_games.index(game)
    #     print("\n")
    #     print(game)
    #     print(index)
    #     if len(list(name_game)) <= 2:
    #         games_score.extend([[game_names[index][0], game_names[index][1], None, None, None]])
    #         print(None)
    #         print("--- %s seconds ---" % (time.time() - start_time), '\n')
    #         # d = {game_names[index][1] : None}
    #         # games_score[game_names[index][0]] = d
    #         # print(games_score[game_names[index][0]])
    #         continue
    #     exit = []
    #     soup, url = search_woking_link(name_game,platform,driver)
    #     if soup != None: #мб ввести условия для слов с одной буквой
    #         try:
    #             max_page = int(soup.find("li", class_ = "page last_page").find("a", class_ = "page_num").text.strip())
    #         except:
    #             max_page = 1
    #         for i in range(max_page):
    #             dict_onerequest = search_gamename_rating(soup) #содержит тайтел и ссылку
    #             for n in range(len(dict_onerequest)):
    #                 list_onerequest = list(dict_onerequest.keys())
    #                 if re.sub(r'[.,"\'-?:!;]', '',name_game.lower()) == re.sub(r'[.,"\'-?:!;]', '',list_onerequest[n].lower()):
    #                     link = dict_onerequest[list_onerequest[n]] #ссылка  на нужную игру
    #                     exit = game
    #                     break
    #             if exit != []:
    #                 break
    ...
    #                 break
    #             url = url + "&page=0"
    #             url = url.replace(f'page={i}', f'page={i + 1}') #если стр 1, то ничего не перезагружаем
    #             driver.get(url) #ждут загрузки видео, где его нет
    #             WebDriverWait(driver, 5).until(ec.element_to_be_clickable(("css selector", "#main_content > div > div.module.search_results.fxdcol.gu6 > div > ul > li.result.first_result > div > div.basic_stats.has_thumbnail > div > h3 > a")))
    #             soup = BeautifulSoup(driver.page_source,"html.parser") #тут не сохр новй пейдж?! мб все ок
    #     else:
    #         link = None
    #     if link == None:
    #         games_score.extend([[game_names[index][0], game_names[index][1], None, None, None]])
    ...
    #         driver.get(link) #меньше времени если будем сортировать по значку медискора
    #         WebDriverWait(driver, 5).until(ec.presence_of_element_located((By.ID , "masthead")))
    #
    #         soup = BeautifulSoup(driver.page_source, "html.parser")
    #         try:
    #             meta_score = float(soup.find(class_ = "section product_scores").find("span").text.strip())
    #             user_score = float(soup.find_all("a", class_="metascore_anchor")[1].find("div").text.strip())
    #             #print(user_score)
    #             #user_score1 =    user_score[1].find("div").text.strip()
    #
    #             release_year = soup.find_all(class_="data")[1].text.strip().replace("  "," ").split(" ")[2]
    #             games_score.extend([[game_names[index][0], game_names[index][1], meta_score, user_score, release_year]])
    #             # d = {game_names[index][1]: [meta_score, user_score, release_year]}
    #             # games_score[game_names[index][0]] = d
    #             print("Игра имеет данные")
    #             print(meta_score, user_score, release_year)
    #             print("--- %s seconds ---" % (time.time() - start_time), '\n')
    #         except:
    #             try:
    #                 user_score = float(soup.find_all("a", class_="metascore_anchor")[0].find("div").text.strip())
    #                 # print(user_score)
    #                 # user_score1 =    user_score[1].find("div").text.strip()
    #
    #                 release_year = soup.find_all(class_="data")[1].text.strip().replace("  ", " ").split(" ")[2]
    #                 games_score.extend(
    #                     [[game_names[index][0], game_names[index][1], None, user_score, release_year]])
    #                 # d = {game_names[index][1]: [meta_score, user_score, release_year]}
    #                 # games_score[game_names[index][0]] = d
    #                 print("Игра имеет данные")
    #                 print(meta_score, user_score, release_year)
    ...
    #                 release_year = soup.find_all(class_="data")[1].text.strip().replace("  ", " ").split(" ")[2]
    #                 games_score.extend(
    #                     [[game_names[index][0], game_names[index][1], None, None, release_year]])
    #                 # d = {game_names[index][1]: [meta_score, user_score, release_year]}
    #                 # games_score[game_names[index][0]] = d
    #                 print("Игра имеет данные")
    #                 print(meta_score, user_score, release_year)
    #                 print("--- %s seconds ---" % (time.time() - start_time), '\n')
    #
    #         # div_class = soup.find("div", class_ = "metascore_w xlarge game positive")
    #         # if div_class == None:
    #         #     d = {game_names[index][1]: None}
    #         #     games_score[game_names[index][0]] = d
    #         #     print(games_score[game_names[index][0]])
    #         # else:
    #         #     meta_score = div_class.find("span").text.strip()
    #         #     user_score = soup.find("a", class_ = "metascore_anchor") #.find("div").text.strip()
    #         #
    #         #     release_year = soup.find_all(class_ = "data")[1].text.strip()
    #         #     release_year = release_year.split(" ")[2]
    #         #     d = {game_names[index][1]: [meta_score,user_score,release_year]}
    #         #     games_score[game_names[index][0]] = d
    #         #     print(game_names[index][0])
    # except:
    #     games_score.extend([[game_names[index][0], game_names[index][1], None, None, None]])
    #     # d = {game_names[index][1]: None}
    #     # games_score[game_names[index][0]] = d
